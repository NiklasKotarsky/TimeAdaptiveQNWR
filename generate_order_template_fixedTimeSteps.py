...
def get_relax_paramFixed(which, dtD,dtN):
    pp = get_parameters(which)
    p2 = {'n': 100, 'dim': 2,'WR_type': 'DNWR', **pp}
    prob = get_problem(**p2)
    
    return prob.DNWR_theta_opt(dtD,dtN)

def getNbrTimeSteps(which, nbrSteps):
    ...
    C1 = max(1, int(D1/D2))
    C2 = max(1, int(D2/D1))
    return C1*nbrSteps,C2*nbrSteps
...
for which in args.testCases: 
    for i in range(0,args.nbrTols):
        ...
        target_path = os.path.join("experiments", str(which),"adaptive",str(tol), str("QuasiNewton"))
        ...
        with open(os.path.join( dirichletPath, "run.sh"), "w") as file:
            file.write(dir.render(tol=tol, case=stuff[0], nbrSteps=-1))
        ...
        with open(os.path.join( neumannPath, "run.sh"), "w") as file:
            file.write(neu.render(tol=tol, case=stuff[1], nbrSteps=-1))
...
for which in args.testCases: 
    for i in range(0,args.nbrTols):
        ...
        target_path = os.path.join("experiments", str(which),"adaptive",str(tol), str("PRelaxation"))
        paths.append(target_path)
        if not os.path.exists(target_path):
            os.makedirs(target_path)
...
precice_config_template = env.get_template('precice-config_fixed.xml')

for which in args.testCases: 
    for i in range(0,args.nbrTols):
        steps = 8*2**(i)

        target_path = os.path.join("experiments", str(which),"fixed",str(steps), str("QuasiNewton"))
        paths.append(target_path)
        if not os.path.exists(target_path):
            os.makedirs(target_path)
        ...
        dirSteps, neuSteps = getNbrTimeSteps(which, steps)

        with open(os.path.join( target_path, precice_config_name), "w") as file:
            file.write(precice_config_template.render())
        ...
        with open(os.path.join( dirichletPath, "run.sh"), "w") as file:
            file.write(dir.render(tol=-1, case=stuff[0],nbrSteps=dirSteps))
        ...
        neu = env.get_template(neumannSolver)
        with open(os.path.join( neumannPath, "run.sh"), "w") as file:
            file.write(neu.render(tol=-1, case=stuff[1],nbrSteps=neuSteps))
...
# precice_config_template = env.get_template('precice-config_fixed_relax.xml')

# for which in args.testCases: 
#     for i in range(0,args.nbrTols):
#         steps = 8*2**(i)

#         target_path = os.path.join("experiments", str(which), "fixed" ,str(steps), str("PRelaxation"))
#         paths.append(target_path)
#         if not os.path.exists(target_path):
#             os.makedirs(target_path)
...
#         with open(os.path.join( target_path, precice_config_name), "w") as file:
#             file.write(precice_config_template.render(tol=1e-10, relax=get_relax_paramFixed(which, 1e4/dirSteps,1e4/neuSteps), timeWindowSize=1e4/steps))
