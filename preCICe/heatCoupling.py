...
## Code in large parts copied from https://github.com/PeterMeisrimel/DNWR
def runRelaxation(self,init_cond, TOL, tf, maxiter=1000, **kwargs):    
    ...
    for k in range(maxiter):
        ...
        while t + dt < tf:
            ...
            u1, ug_new, flux2,err1 = self.solve_dirichlet_SDIRK2(dt, u1,ug_new, f)
        ...
        u1, ug_new, flux2, _ = self.solve_dirichlet_SDIRK2(dt, u1,ug_new, f)
        t1.append(tf)
        ...
        self.r_old = TOL_N  # for PI controller
        ...
        while t + dt < tf:
            ...
            u2, ug_new, _, err2 = self.solve_neumann_SDIRK2(dt,u2,ug_old,f)
            t += dt
            t2_new.append(t)
    ...
    with open("resultsRelaxation_" + str(TOL), 'w') as myfile:
        myfile.write(json.dumps(results, indent=2, sort_keys=True))



def runParticipant(self, dirichlet,init_cond, TOL, tf, **kwargs):
    ...
    print("../resultsQN_" + str(TOL) + "_dir" + str(dirichlet))
    with open("../resultsQN_" + str(TOL) + "_dir" + str(dirichlet), 'w') as myfile:
        myfile.write(json.dumps(results, indent=2, sort_keys=True))
