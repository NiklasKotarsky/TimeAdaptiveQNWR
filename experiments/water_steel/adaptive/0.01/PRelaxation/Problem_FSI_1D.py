...
class Problem_FSI_1D(Problem_FSI):
    ...
    def __init__(self, n=10, lambda_1=0, lambda_2=0, alpha_1=0, alpha_2=0, WR_type='DNWR', len_1=1, len_2=1):
        super(Problem_FSI_1D, self).__init__(n, lambda_1, lambda_2,
    ...
    def compute_matrices(self, nx, ny, alpha, lambda_diff, interface, neumann=False):
        ...
        Aig = sp.lil_matrix((n, 1))
        Aig[intf_idx, 0] = -a_fac
        Aig.tocsr()
        Agi = sp.lil_matrix((1, n))
        Agi[0, intf_idx] = -a_fac
        Agi.tocsr()
        Aggi = sp.spdiags(a_fac*np.ones(1), 0, 1, 1, format='csr')
        Ai = sp.spdiags([2*a_fac*np.ones(n), -a_fac*np.ones(n), -
                        a_fac*np.ones(n)], [0, 1, -1], n, n, format='csr')

        Mig = sp.lil_matrix((n, 1))
        Mig[intf_idx, 0] = alpha/6
        Mig.tocsr()
        Mgi = sp.lil_matrix((1, n))
        Mgi[0, intf_idx] = alpha/6
        Mgi.tocsr()
        Mggi = sp.spdiags(alpha/3*np.ones(1), 0, 1, 1, format='csr')
        Mi = sp.spdiags([alpha*4/6*np.ones(n), alpha/6*np.ones(n),
                        alpha/6*np.ones(n)], [0, 1, -1], n, n, format='csr')
        ...
        self.der = sp.spdiags([(-lambda_diff)],
                              [-intf_idx], 1, n, format='csr')
