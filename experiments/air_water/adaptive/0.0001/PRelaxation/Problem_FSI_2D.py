...
class Problem_FSI_2D(Problem_FSI):
    ...
    def __init__(self, n=10, lambda_1=0, lambda_2=0, alpha_1=0, alpha_2=0, WR_type='DNWR', len_1=1, len_2=1):
        super(Problem_FSI_2D, self).__init__(n, lambda_1, lambda_2,
                                             alpha_1, alpha_2, WR_type=WR_type, len_1=len_1, len_2=len_2)
    ...
    def compute_matrices(self, nx, ny, alpha, lambda_diff, interface, neumann=False):
        ...
        if interface == 'right':
            ...
            raise ValueError('invalid interface position')
        ...
        Aggi = sp.spdiags([2*a_fac*np.ones(ny)] + 2*[-1/2 *
                          a_fac*np.ones(ny)], [0, 1, -1], ny, ny, format='csr')
        ...
        Mig = sp.spdiags([-alpha/12*np.ones(NN), alpha/4*np.ones(NN)],
                         [intf_idx, intf_idx + intf_idx_plus], NN, ny, format='csr')
        Mgi = sp.spdiags([-alpha/12*np.ones(NN), alpha/4*np.ones(NN)],
                         [-intf_idx, -intf_idx + intf_idx_plus], ny, NN, format='csr')
        Mggi = sp.spdiags([5*alpha/12*np.ones(ny)] + 2 *
                          [-alpha/24*np.ones(ny)], [0, 1, -1], ny, ny, format='csr')

        N = sp.spdiags([5/6*np.ones(ny)] + 2*[-1/12*np.ones(ny)],
                       [0, 1, -1], ny, ny, format='csr')
        N1 = sp.spdiags([-1/12*np.ones(ny), 1/4*np.ones(ny)],
                        [0, -1], ny, ny, format='csr')
        N2 = sp.spdiags([-1/12*np.ones(ny), 1/4*np.ones(ny)],
                        [0, 1], ny, ny, format='csr')
        Mi = alpha*(sp.kron(sp.spdiags(np.ones(nx), 0, nx, nx, format='csr'), N)
                    + sp.kron(sp.spdiags(np.ones(nx), -1,
                              nx, nx, format='csr'), N1)
                    + sp.kron(sp.spdiags(np.ones(nx), 1, nx, nx, format='csr'), N2))

        self.lambda_diff = lambda_diff
        self.der = sp.spdiags([(-lambda_diff)*np.ones(NN)],
                              [-intf_idx], ny, NN, format='csr')
    ...
    def compute_iteration_matrix_two_step(self, dt):
        ...
        S1 = (self.Mg1 + dt*self.Ag1)@sp.linalg.inv(self.M1 +
                                                    dt * self.A1)@(self.M1g + dt*self.A1g)
        S2Inv = sp.linalg.inv(
            (self.Mg2 + dt*self.Ag2) @ sp.linalg.inv(self.M2 + dt * self.A2) @ (self.M2g + dt*self.A2g))

        part1 = self.Mg1 @ sp.linalg.inv(self.M1 +
                                         dt * self.A1)@(self.M1g + dt*self.A1g)
        part2 = self.Mg2 @ sp.linalg.inv(self.M2 + dt *
                                         self.A2) @ (self.M2g + dt*self.A2g)@S2Inv@S1

        part3 = (self.Mg1 + dt*self.Ag1)@sp.linalg.inv(self.M1 + dt *
                                                       self.A1) @ self.M1 @ sp.linalg.inv(self.M1 + dt * self.A1)@(self.M1g + dt*self.A1g)

        part4 = (self.Mg2 + dt*self.Ag2) @ sp.linalg.inv(self.M2 + dt *
                                                         self.A2) @ self.M2 @ sp.linalg.inv(self.M2 + dt *
                                                                                            self.A2) @ (self.M2g + dt*self.A2g) @ S2Inv @ S1

        part5 = (self.Mg2 + dt*self.Ag2) @ sp.linalg.inv(self.M2 +
                                                         dt * self.A2) @ (self.M2g)@S2Inv@S1

        part6 = (self.Mgg1 + self.Mgg2)@S2Inv@S1
        off_diag = -S2Inv@(part1 - part2 - part3 - part4 - part5 + part6)
        M = sp.bmat([[-S2Inv@S1, None],
                     [off_diag, -S2Inv@S1]], format='csr')
        return M
    ...
    def get_initial_values(self, init_cond):
        ...
        u1, u2, ug = np.zeros(self.n_int_1), np.zeros(
            self.n_int_2), np.zeros(n)
        for i in range(n):
            ug[i] = init_cond(0., y[i+1])
        for i in range(n):
