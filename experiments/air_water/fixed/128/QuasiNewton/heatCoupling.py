...
def runParticipant(self, dirichlet,init_cond, TOL, tf, nbrSteps = None, **kwargs):
    ...
    if dirichlet:
        ...
        self.r_old = TOL_D
        if TOL != -1:
            solverDt = self.get_dt0(tf, TOL_D, u10, which=1)
        else:
            solverDt = tf/nbrSteps
        domain = 'D'
        ...
        self.r_old = TOL_N
        if TOL != -1:
            solverDt = self.get_dt0(tf, TOL_N, u10, which=2)
        else:
            solverDt = tf/nbrSteps
        domain = 'N'
    ...
    while participant.is_coupling_ongoing():
        ...
        if TOL != -1:
            solverDt = self.get_new_dt_PI(dt, self.norm_inner(
                    le, domain), TOL_D)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-nbrSteps", "--nbrSteps", help="nbrOfTimeSteps", default=None,type=float)
    ...
    if args.nbrSteps is None:
        ...
        if not args.relax:
            prob.runParticipant(dirichlet,init_cond_2d, TOL=args.Tol, tf= args.Tf, nbrSteps=args.nbrSteps)